                ...
                <table width="650" class="break" cellspacing="0" cellpadding="0" >
                  ...
                  <tr>
                    <td valign="top" style="padding:15px;">
					  ...
					  @endphp
                        <tr>
                          <td colspan="4"><font size="2"><b><u>Monthly Invoice - #{{$invoiceId}}</u></b></font><br><br></td>
                        </tr>
                        <tr>
						<td colspan="4">
						<div style="float:left;">Company:<br>Contact:<br>Address:<br><br><br><br><br>Period:</div>
						<div style="float:left; padding-left:15px; line-height:16ps;"><b>{{ucfirst($customer->defaultaddress->company_name)}}<br>{{ucfirst($customer->first_name.' '.$customer->last_name)}}<br>{{$customer->defaultaddress->street_address}}<br>{{$customer->defaultaddress->address_line_2}}<br>{{$customer->defaultaddress->city}}<br>{{$customer->defaultaddress->county_state}}<br><br>{{$m_start}} - {{$m_end}}</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Customer No:<b>#{{$customer->id}}</b>
						</div>
                      ...
                      <p>
                      <form id="emailinvoice" action="{{route('admin.reports.mail-monthly-invoice')}}" method="post">
					  @csrf
                        ...
                        <p>
                          <input type="hidden" id="month" name="month" value="{{$month_no}}" />
