...
class CategoryRepository extends BaseRepository implements CategoryRepositoryInterface
{
    ...
    public function createCategory(array $params) : Category
    {
        try {
            ...
            if (isset($params['cover']) && ($params['cover'] instanceof UploadedFile)) {
				$nameArr = explode('.', $params['cover']->getClientoriginalName());
				$ext = $params['cover']->getClientOriginalExtension();
				
				$fileName = $nameArr[0].'__'.time().$ext;
				
				$cover = $this->uploadOne($params['cover'], 'categories','uploads',$fileName);
            }
    ...
    public function deleteFile(array $file, $disk = null) : bool
    {
		...
		$image_cat_path = public_path().'/uploads/'.$category->cover;
	
		if(file_exists($image_cat_path)){
			@unlink($image_cat_path);
		}
		
        return $category->update(['cover' => null]);
    }
     ...
     * @return mixed
     */
	public function listParentCategories(string $order = 'id', string $sort = 'desc', $except = []) : Collection
    {
        return $this->model->orderBy($order, $sort)->where('parent_id', 0)->get()->except($except);
    }
	
	/**
     * @return mixed
     */
	public function listSubCategories($category_id, string $order = 'id', string $sort = 'desc', $except = []) : Collection
    {
        return $this->model->orderBy($order, $sort)->where('parent_id', $category_id)->get()->except($except);
    }
