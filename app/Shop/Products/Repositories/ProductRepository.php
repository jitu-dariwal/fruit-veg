...
class ProductRepository extends BaseRepository implements ProductRepositoryInterface
{
    ...
    {
		$product = $this->findProductById($file['product']);
		
		$image_path = public_path().'/uploads/'.$product->cover;
	
		if(file_exists($image_path)){
			@unlink($image_path);
		}
		
		return DB::table('products')->where('id', $file['product'])->update(['cover' => null]);
    ...
    {
        return $file->store('products', ['disk' => 'uploads']);
    }
    ...
    public function saveProductImages(Collection $collection)
    {
        $collection->each(function (UploadedFile $file) {
            $filename = $this->storeFile($file,'products','uploads');
