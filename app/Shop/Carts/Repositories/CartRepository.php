...
class CartRepository extends BaseRepository implements CartRepositoryInterface
{
     ...
     * @return float
     */
    public function getTotal(int $decimals = 2, $shipping = 0.00, $discount = 0.00)
    {
        return $this->model->total($decimals, null, null, $shipping, $discount);
    }
    ...
    public function getCartItemsTransformed() : Collection
    {
        return $this->getCartItems()->map(function ($item) {
            ...
            //$item->catname = $product->catname;
            $item->product_id = $product->id;
            $item->type = $product->type;
            $item->packet_size = $product->packet_size;
